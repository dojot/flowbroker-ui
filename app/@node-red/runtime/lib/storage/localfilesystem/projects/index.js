...
const storageSettings = require("../settings");
...
function init(_settings, _runtime) {
  ...
  if (projectsEnabled) {
    return sshTools.init(settings, runtime).then(() => {
      gitTools.init(_settings).then((gitConfig) => {
        if (!gitConfig || /^1\./.test(gitConfig.version)) {
          if (!gitConfig) {
            projectLogMessages.push(log._("storage.localfilesystem.projects.git-not-found"));
          } else {
            projectLogMessages.push(
              log._("storage.localfilesystem.projects.git-version-old", {
...
function listProjects() {
  return fs.readdir(projectsDir).then((fns) => {
    const dirs = [];
    fns
...
function loadProject(name) {
  ...
  return Projects.load(projectPath).then((project) => {
...
function checkActiveProject(project) {
  if (!activeProject || activeProject.name !== project) {
    ...
    throw new Error(
      `Cannot operate on inactive project wanted:${project} current:${
...
function revertFile(user, project, filePath) {
  checkActiveProject(project);
  return activeProject.revertFile(filePath).then(() => reloadActiveProject("revert"));
}
...
function pull(user, project, remoteBranchName, setRemote, allowUnrelatedHistories) {
  ...
  return activeProject
    .pull(user, remoteBranchName, setRemote, allowUnrelatedHistories)
    .then(() => reloadActiveProject("pull"));
}
...
function abortMerge(user, project) {
  checkActiveProject(project);
  return activeProject.abortMerge().then(() => reloadActiveProject("merge-abort"));
}
...
function setBranch(user, project, branchName, isCreate) {
  checkActiveProject(project);
  return activeProject
    .setBranch(branchName, isCreate)
    .then(() => reloadActiveProject("change-branch"));
}
...
function reloadActiveProject(action) {
  return runtime.nodes.stopFlows().then(() =>
    runtime.nodes
      .loadFlows(true)
      .then(() => {
        events.emit("runtime-event", {
          id: "project-update",
          payload: { project: activeProject.name, action },
        });
      })
      .catch((err) => {
...
function createProject(user, metadata) {
  ...
  return Projects.create(user, metadata)
    .then((p) => setActiveProject(user, p.name))
    .then(() => getProject(user, metadata.name));
}
function setActiveProject(user, projectName) {
  return loadProject(projectName).then((project) => {
    const globalProjectSettings = settings.get("projects") || {};
    globalProjectSettings.activeProject = project.name;
    return settings.set("projects", globalProjectSettings).then(() => {
      log.info(
        log._("storage.localfilesystem.projects.changing-project", {
...
function initialiseProject(user, project, data) {
  ...
  return activeProject.initialise(user, data).then((result) => {
...
function updateProject(user, project, data) {
  ...
  return activeProject.update(user, data).then((result) => {
    ...
    if (result.credentialSecretChanged) {
...
function setCredentialSecret(data) {
  ...
  const isReset = data.resetCredentialSecret;
  const wasInvalid = activeProject.credentialSecretInvalid;
  return activeProject.update(data).then(() => {
    if (isReset || !wasInvalid) {
      if (isReset) {
        runtime.nodes.clearCredentials();
      }
...
async function getFlows() {
  ...
  if (!initialFlowLoadComplete) {
    ...
    if (projectsEnabled) {
      log.info(
        log._("storage.localfilesystem.projects.projects-directory", {
    ...
    if (activeProject) {
      ...
      return loadProject(activeProject).then(() => {
        log.info(
          log._("storage.localfilesystem.projects.active-project", {
  ...
  return util.readFile(flowsFullPath, flowsFileBackup, null, "flow").then((result) => {
    if (result === null) {
    ...
    if (activeProject) {
      ...
      if (workflowMode === "auto") {
        return activeProject.stageFile([flowsFullPath, credentialsFile]).then(() =>
          activeProject.status(user, false).then((result) => {
