            ...
            try {
              ...
              if (project.package.hasOwnProperty("node-red")) {
                if (project.package["node-red"].hasOwnProperty("settings")) {
                  project.paths.flowFile = fspath.join(
                    project.paths.root,
                    project.package["node-red"].settings.flowFile,
                  );
                  project.paths.credentialsFile = fspath.join(
                    project.paths.root,
                    project.package["node-red"].settings.credentialsFile,
                  );
                }
              } else {
...
Project.prototype.update = async function (user, data) {
  ...
  if (data.hasOwnProperty("files")) {
    ...
    if (
      ...
      (data.files.package !== fspath.join(this.paths.root, "package.json") ||
        !this.paths["package.json"])
    ) {
    ...
    if (
      ...
      this.package["node-red"].settings.flowFile !==
        data.files.flow.substring(this.paths.root.length)
    ) {
      ...
      this.package["node-red"].settings.flowFile = data.files.flow.substring(
    ...
    if (
      ...
      this.package["node-red"].settings.credentialsFile !==
        data.files.credentials.substring(this.paths.root.length)
    ) {
      if (
        /^\.\./.test(fspath.relative(this.path, fspath.join(this.path, data.files.credentials)))
      ) {
        return Promise.reject("Invalid credentials file: " + data.files.credentials);
      }
      this.paths.credentialsFile = data.files.credentials;
      this.package["node-red"].settings.credentialsFile = data.files.credentials.substring(
        this.paths.root.length,
      );
  ...
  if (data.hasOwnProperty("git")) {
    if (data.git.hasOwnProperty("user")) {
      globalProjectSettings.projects[this.name].git =
        globalProjectSettings.projects[this.name].git || {};
      globalProjectSettings.projects[this.name].git.user =
        globalProjectSettings.projects[this.name].git.user || {};
  ...
  if (saveREADME) {
    promises.push(
      util
        .writeFile(fspath.join(this.path, this.paths["README.md"]), this.description)
        .catch((err) => {}),
    );
      ...
      fs
        ...
        .then((content) => {
          ...
          return util.writeFile(
            fspath.join(project.path, this.paths["package.json"]),
            JSON.stringify(this.package, "", 4),
          );
        })
        .catch((err) => {}),
    );
  ...
  return Promise.all(promises)
    .then(function (res) {
      var gitSettings = getUserGitSettings(user) || {};
      var workflowMode =
        (gitSettings.workflow || {}).mode || settings.editorTheme.projects.workflow.mode;
  ...
  if (remoteName) {
    return gitTools
      .fetch(
        project.path,
        remoteName,
        authCache.get(project.name, project.remotes[remoteName].fetch, username),
      )
      .catch(function (err) {
    ...
    remotes.forEach(function (remote) {
      promise = promise
        .then(function () {
          return gitTools.fetch(
            project.path,
            remote,
            authCache.get(project.name, project.remotes[remote].fetch, username),
          );
        })
        .catch(function (err) {
          if (!err.remote) {
...
Project.prototype.updateRemote = function (user, remote, options) {
  ...
  if (options.auth) {
    ...
    if (options.auth.keyFile) {
      options.auth.key_path = sshKeys.getPrivateKeyPath(
        getSSHKeyUsername(user),
        options.auth.keyFile,
      );
    }
  ...
  return {
    ...
    settings: {
      credentialsEncrypted:
        typeof this.credentialSecret === "string" && this.credentialSecret.length > 0,
...
function createDefaultProject(user, project) {
  ...
  return gitTools.initRepo(projectPath).then(function () {
    ...
    if (project.files) {
      if (project.files.flow && !/\.\./.test(project.files.flow)) {
        ...
        if (project.migrateFiles) {
          ...
          var baseCredentialFileName =
            project.files.credentials || fspath.basename(project.files.oldCredentials);
          ...
          project.files.credentials =
            project.files.credentials || getCredentialsFilename(project.files.flow);
          files.push(project.files.flow);
          files.push(project.files.credentials);
    ...
    for (var file in defaultFileSet) {
      if (defaultFileSet.hasOwnProperty(file)) {
        promises.push(
          util.writeFile(fspath.join(projectPath, file), defaultFileSet[file](project)),
        );
      }
    }
      ...
      fs.ensureDir(projectPath)
        ...
        .then(function () {
          if (metadata.git && metadata.git.remotes && metadata.git.remotes.origin) {
            ...
            var auth;
            if (
              originRemote.hasOwnProperty("username") &&
              originRemote.hasOwnProperty("password")
            ) {
              ...
              originRemote.hasOwnProperty("keyFile") &&
              originRemote.hasOwnProperty("passphrase")
            ) {
              authCache.set(project, originRemote.url, username, {
                // TODO: hardcoded remote name
