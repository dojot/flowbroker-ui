...
function loadFile(storagePath) {
  return fs.pathExists(storagePath).then((exists) => {
    if (exists === true) {
      ...
      files.forEach((file) => {
        if (!/^\./.test(file)) {
          ...
          if (stats.isDirectory()) {
            promises.push(
              fs.readdir(fullPath).then((subdirFiles) => {
                const result = [];
...
function stringify(value) {
  let hasCircular;
  const result = safeJSONStringify(value, null, 4, (k, v) => {
    hasCircular = true;
  });
        ...
        scopes.forEach((scope, i) => {
          try {
            ...
            const error = new Error(
              log._("context.localfilesystem.invalid-json", { file: contextFiles[i] }),
            );
            throw error;
          }
        });
...
LocalFileSystem.prototype.close = function () {
  ...
  if (this.cache && this._pendingWriteTimeout) {
    ...
    self.writePromise = self.writePromise.then(() =>
      self._flushPendingWrites.call(self).catch((err) => {
...
LocalFileSystem.prototype.set = function (scope, key, value, callback) {
  ...
  if (this.cache) {
    ...
    self.writePromise = self.writePromise
      .then(() => loadFile(`${storagePath}.json`))
      .then((data) => {
