    ...
    runtimeLibraries.forEach((library) => {
      if (library.type === id) {
        ...
        if (!/^[a-z0-9-_]+$/.test(library.id)) {
          log.warn(
            log._("library.failedToInit", {
              error: log._("library.invalidProperty", { prop: "id", value: library.id }),
            }),
          );
          return;
        }
        try {
          ...
          if (libraries[library.id].init) {
            libraries[library.id].init().catch((err) => {
              ...
              log.warn(
                log._("library.failedToInit", { library: library.id, error: err.toString() }),
              );
            });
          }
        } catch (err) {
