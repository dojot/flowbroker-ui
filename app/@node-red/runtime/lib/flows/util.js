...
const typeRegistry = require("../../../registry/lib");
...
function diffNodes(oldNode, newNode) {
  ...
  const oldKeys = Object.keys(oldNode).filter((p) => p != "x" && p != "y" && p != "wires");
  const newKeys = Object.keys(newNode).filter((p) => p != "x" && p != "y" && p != "wires");
...
function mapEnvVarProperties(obj, prop, flow) {
  ...
  if (Buffer.isBuffer(v)) {
    ...
    if (obj[prop][0] === "$" && (EnvVarPropertyRE_old.test(v) || EnvVarPropertyRE.test(v))) {
      ...
      const r = flow.getSetting(envVar);
...
function createNode(flow, config) {
  ...
  const { type } = config;
  try {
    const nodeTypeConstructor = typeRegistry.get(type);
    if (typeof nodeTypeConstructor === "function") {
      ...
      const subflowConfig = parseConfig(
        [nodeTypeConstructor.subflow].concat(nodeTypeConstructor.subflow.flow),
      );
      ...
      const subflow = require("./Subflow").createModuleInstance(
      ...
      Log.error(Log._("nodes.flow.unknown-type", { type }));
    }
  } catch (err) {
    Log.error(err);
  }
...
function parseConfig(config) {
  ...
  config.forEach((n) => {
    if (n.type !== "subflow" && n.type !== "tab" && n.type !== "group") {
      ...
      if (
        (subflowDetails && !flow.subflows[subflowDetails[1]]) ||
...
module.exports = {
  init(runtime) {
    envVarExcludes = {};
    if (
      runtime.settings.hasOwnProperty("envVarExcludes") &&
      Array.isArray(runtime.settings.envVarExcludes)
    ) {
