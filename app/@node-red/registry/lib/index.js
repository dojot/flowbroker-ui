...
function enableNodeSet(typeOrId) {
  return registry.enableNodeSet(typeOrId).then(() => {
    const nodeSet = registry.getNodeInfo(typeOrId);
    if (!nodeSet.loaded) {
      return loader
        .loadNodeSet(registry.getFullNodeInfo(typeOrId))
        .then(() => registry.getNodeInfo(typeOrId));
    }
