...
const subflows = require("./subflow");
const externalModules = require("./externalModules");

const logger = new Logger("flowbroker-ui:registry/lib/registry");
  ...
  for (const module in moduleConfigs) {
    ...
    if (moduleConfigs.hasOwnProperty(module)) {
      if (Object.keys(moduleConfigs[module].nodes).length > 0) {
        ...
        const { nodes } = moduleConfigs[module];
...
function getNodeList(filter) {
  ...
  for (const module in moduleConfigs) {
    ...
    if (moduleConfigs.hasOwnProperty(module)) {
      if (
        !moduleConfigs[module].user &&
        moduleConfigs[module].usedBy &&
        moduleConfigs[module].usedBy.length > 0
      ) {
        continue;
      }
      const { nodes } = moduleConfigs[module];
...
function registerNodeConstructor(nodeSet, type, constructor, options) {
  logger.debug(`Registering a new node: ${type}`);
...
function enableNodeSet(typeOrId) {
  ...
  try {
    ...
    return saveNodeList().then(() => filterNodeInfo(config));
  } catch (err) {
...
function setModulePendingUpdated(module, version) {
  ...
  return saveNodeList().then(() => getModuleInfo(module));
}
