...
function createNodeApi(node) {
  // Avoiding duplicated node instantiation that leads to an error
  if (global.nodeStorage[node.id]) {
    global.nodeStorage[node.id].alreadyStored = true;
    return global.nodeStorage[node.id];
  }
  var red = {
    nodes: {},
    log: {},
  ...
  global.nodeStorage[node.id] = red;
  return red;
}
