...
RED.palette = (function () {
  ...
  function createCategoryContainer(originalCategory, category, labelId) {
    ...
    const catDiv = $(`<div id="red-ui-palette-container-${category}" class="red-ui-palette-category hide">`
    ...
    $(`#red-ui-palette-header-${category}`).on("click", (e) => {
  ...
  function setLabel(type, el, label, info) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const sep = (i == 0) ? "" : " ";
      const newWidth = RED.view.calculateTextWidth(currentLine + sep + word, "red-ui-palette-label");
      if (newWidth < nodeWidth) {
        ...
        while (true) {
          const wordWidth = RED.view.calculateTextWidth(word, "red-ui-palette-label");
          if (wordWidth >= nodeWidth) {
            // break word if too wide
            for (let j = word.length; j > 0; j--) {
              const s = word.substring(0, j);
              const width = RED.view.calculateTextWidth(s, "red-ui-palette-label");
              if (width < nodeWidth) {
                displayLines.push(s);
    ...
    const labelElement = el.find(".red-ui-palette-label");
    ...
    let popOverContent;
    try {
      const l = `<p><b>${RED.text.bidi.enforceTextDirectionWithUCC(label)}</b></p>`;
      popOverContent = $("<div></div>").append($(l + (info || RED.nodes.getNodeHelp(type) || `<p>${RED._("palette.noInfo")}</p>`).trim())
        .filter(function (n) {
      ...
      const typeInfo = RED.nodes.getType(type);
  ...
  function setIcon(element, sf) {
    const icon_url = RED.utils.getNodeIcon(sf._def);
    const iconContainer = element.find(".red-ui-palette-icon-container");
    const currentIcon = iconContainer.attr("data-palette-icon");
  ...
  function addNodeType(nt, def) {
    ...
    const nodeCategory = def.category;

    if (exclusion.indexOf(nodeCategory) === -1) {
      const originalCategory = nodeCategory;
      const category = escapeCategory(nodeCategory);
      const rootCategory = category.split("-")[0];

      const d = $("<div>", { class: "red-ui-palette-node" }).attr("data-palette-type", nt).data("category", rootCategory);

      let label = nt;/// ^(.*?)([ -]in|[ -]out)?$/.exec(nt)[1];
      ...
      $("<div/>", {
        class: `red-ui-palette-label${((!def.align && def.inputs !== 0 && def.outputs === 0) || def.align === "right") ? " red-ui-palette-label-right" : ""}`
      }).appendTo(d);


      if (def.icon) {
        const icon_url = RED.utils.getNodeIcon(def);
        const iconContainer = $("<div/>", {
          class: `red-ui-palette-icon-container${((!def.align && def.inputs !== 0 && def.outputs === 0) || def.align === "right") ? " red-ui-palette-icon-container-right" : ""}`
        }).appendTo(d);
        iconContainer.attr("data-palette-icon", icon_url);
        RED.utils.createIconElement(icon_url, iconContainer, true);
      }
      ...
      d.on("mousedown", (e) => { e.preventDefault(); });

      const popover = RED.popover.create({
        target: d,
        trigger: "hover",
        interactive: true,
        width: "300px",
      ...
      const chartSVG = $("#red-ui-workspace-chart>svg").get(0);
      ...
      $(d).draggable({
        ...
        drag(e, ui) {
          const paletteNode = getPaletteNode(nt);
          ui.originalPosition.left = paletteNode.offset().left;
          mouseX = ui.position.left - paletteWidth + (ui.helper.width() / 2) + chart.scrollLeft();
          mouseY = ui.position.top - paletteTop + (ui.helper.height() / 2) + chart.scrollTop() + 10;
          if (!groupTimer) {
            groupTimer = setTimeout(() => {
              const mx = mouseX / RED.view.scale();
              const my = mouseY / RED.view.scale();
              const group = RED.view.getGroupAtPoint(mx, my);
              if (group !== hoverGroup) {
                if (hoverGroup) {
                  document.getElementById(`group_select_${hoverGroup.id}`).classList.remove("red-ui-flow-group-hovered");
                }
      ...
      if (nt.indexOf("subflow:") === 0) {
        ...
        const subflow = RED.nodes.subflow(nt.substring(8));
        nodeInfo = RED.utils.renderMarkdown(subflow.info || "");
      }
      setLabel(nt, d, label, nodeInfo);

      const categoryNode = $(`#red-ui-palette-container-${rootCategory}`);
  ...
  function removeNodeType(nt) {
    const paletteNode = getPaletteNode(nt);
    const categoryNode = paletteNode.closest(".red-ui-palette-category");
    paletteNode.remove();
  ...
  function hideNodeType(nt) {
    ...
    paletteNode.hide();
    const categoryNode = paletteNode.closest(".red-ui-palette-category");
    const cl = categoryNode.find(".red-ui-palette-node");
    let c = 0;
  ...
  function refreshSubflow(sf) {
    const paletteNode = getPaletteNode(`subflow:${sf.id}`);
    const portInput = paletteNode.find(".red-ui-palette-port-input");
    const portOutput = paletteNode.find(".red-ui-palette-port-output");

    const paletteLabel = paletteNode.find(".red-ui-palette-label");
    paletteLabel.attr("class", `red-ui-palette-label${((!sf._def.align && sf.in.length !== 0 && sf.out.length === 0) || sf._def.align === "right") ? " red-ui-palette-label-right" : ""}`);

    const paletteIconContainer = paletteNode.find(".red-ui-palette-icon-container");
    paletteIconContainer.attr("class", `red-ui-palette-icon-container${((!sf._def.align && sf.in.length !== 0 && sf.out.length === 0) || sf._def.align === "right") ? " red-ui-palette-icon-container-right" : ""}`);
    ...
    const currentLabel = paletteNode.attr("data-palette-label");
    const currentInfo = paletteNode.attr("data-palette-info");
    ...
    const currentCategory = paletteNode.data("category");
    const newCategory = (sf.category || "subflows");
    if (currentCategory !== newCategory) {
      const category = escapeCategory(newCategory);
      createCategory(newCategory, category, category, "node-red");

      const currentCategoryNode = paletteNode.closest(".red-ui-palette-category");
      const newCategoryNode = $(`#red-ui-palette-${category}`);
  ...
  function filterChange(val) {
    ...
    $("#red-ui-palette-container .red-ui-palette-node").each(function (i, el) {
      const currentLabel = $(el).attr("data-palette-label");
      const type = $(el).attr("data-palette-type");
  ...
  function init() {
    ...
    $("#red-ui-palette").on("mouseenter", () => {
      sidebarControls.toggle("slide", { direction: "left" }, 200);
    });
    $("#red-ui-palette").on("mouseleave", () => {
    ...
    const paletteFooterButtons = $("<span class=\"button-group\"></span>").appendTo("#red-ui-palette .red-ui-component-footer");
    const paletteCollapseAll = $("<button type=\"button\" class=\"red-ui-footer-button\"><i class=\"fa fa-angle-double-up\"></i></button>").appendTo(paletteFooterButtons);
    paletteCollapseAll.on("click", (e) => {
      e.preventDefault();
    ...
    const paletteExpandAll = $("<button type=\"button\" class=\"red-ui-footer-button\"><i class=\"fa fa-angle-double-down\"></i></button>").appendTo(paletteFooterButtons);
    paletteExpandAll.on("click", (e) => {
      e.preventDefault();
      for (const cat in categoryContainers) {
        if (categoryContainers.hasOwnProperty(cat)) {
    ...
    RED.actions.add("core:toggle-palette", (state) => {
      if (state === undefined) {
        RED.menu.toggleSelected("menu-item-palette");
      } else {
        togglePalette(state);
  ...
  function getCategories() {
    const categories = [];
    $("#red-ui-palette-container .red-ui-palette-category").each((i, d) => {
